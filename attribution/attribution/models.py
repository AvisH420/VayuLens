...
class GridCellInput(BaseModel):
    ...
    road_density: float = Field(..., description="Road density score representing traffic traffic-heavy areas")
...
class SourceAttributionOutput(BaseModel):
    ...
    # Share percentages (should sum to 1.0)
    traffic_share: float = Field(..., ge=0.0, le=1.0, description="Fraction of PM2.5 attributed to vehicular traffic")
    industrial_share: float = Field(..., ge=0.0, le=1.0, description="Fraction of PM2.5 attributed to industrial emissions")
    biomass_share: float = Field(..., ge=0.0, le=1.0, description="Fraction of PM2.5 attributed to biomass burning (e.g. stubble)")
    dust_share: float = Field(..., ge=0.0, le=1.0, description="Fraction of PM2.5 attributed to dust (road/soil/desert)")
    construction_share: float = Field(..., ge=0.0, le=1.0, description="Fraction of PM2.5 attributed to construction activity")
    other_share: float = Field(..., ge=0.0, le=1.0, description="Fraction of PM2.5 attributed to other unclassified sources")
